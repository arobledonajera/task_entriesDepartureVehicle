...
const { entDepVehiclesServices, configurationService } = require('../services/index.service');
...
const taskEntriesDepartures = async () => {
  const { groupIdTrailers, database } = await configurationService.getConfiguration();
  ...
  const geotabDevices = await geotabApi.getDevices(groupIdTrailers);
        ...
        if (res.type == 'entrie') {
          ...
          if (dateEntrie != '' && identifier != '' && res.DeviceId == deviceId) {
            if (res.identifier == identifier) {
              ...
              const insert = await entDepVehiclesServices.addDurationEntriesDepartures(
                res.DeviceId,
                res.economic,
                res.vin,
                res.plate,
              ...
              (identifier = ''), (dateEntrie = ''), (dateDeparture = ''), (duration = ''), (subtraction = '');
            }
          }
        }
      }
